...
// GET /FE/reminders/:userId

export const FEGetReminders = (req, res) => {
  const { userId } = req.params;
  const { startDate, endDate, status = "all" } = req.query;
...
// POST /FE/reminders/:userId

export const FECreateReminder = (req, res) => {
  const { userId } = req.params;
  const {
    medicineName,
    medicineDetails,
...
// PUT /FE/reminders/:reminderId

export const FEUpdateReminder = (req, res) => {
  const { reminderId } = req.params;
  const {
    medicineName,
    medicineDetails,
...
// DELETE /FE/reminders/:reminderId

export const FEDeleteReminder = (req, res) => {
  const { reminderId } = req.params;
  db.query(
    `DELETE FROM tr_reminder_times WHERE remind_medicine_id = ?`,
    [reminderId],
...
// POST /FE/reminders/:reminderId/taken

export const FEMarkTaken = (req, res) => {
  const { reminderId } = req.params;
  const { date, time } = req.body;
  if (!date || !time) {
    return res.status(400).json({
